...
function App() {
  ...
  const handleConvert = async () => {
    ...
    if (!aiReady) {
      setFeedback("❌ AI not ready yet. Please wait...");
      return;
    }
    setLoading(true);
    setFeedback("");
    ...
    try {
      const res = await window.puter.ai.chat(
        `Convert the following JavaScript code to ${targetLang}. Only return the converted code with no explanations.
        Code: ${inputCode}`
      );
      const reply =
        typeof res === "string"
      ...
      setFeedback("✅ Code converted successfully!");
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-950 via-slate-950 to-purple-950 flex flex-col items-center justify-center p-6 gap-10 relative overflow-hidden">
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-center items-center relative z-10">
        ...
        >
          {["Python", "Java", "C++", "Go", "JavaScript", "TypeScript"].map(
            (lang) => (
        ...
        <button
          ...
          className="px-6 py-3 bg-gradient-to-r from-violet-500 to-orange-500 hover:opacity-80 active:scale-95 text-white font-semibold rounded-2xl transition-all flex items-center gap-2 disabled:opacity-50 shadow-lg cursor-pointer"
        >
          {loading ? (
        ...
        <button
          onClick={handleReset}
          disabled={loading}
          className="px-6 py-3 bg-gradient-to-r from-rose-500 to-cyan-500 hover:opacity-80 active:scale-95 text-white font-semibold rounded-2xl transition-all flex items-center gap-2 disabled:opacity-50 shadow-lg cursor-pointer"
        >
          <RotateCcw className="w-5 h-5" /> Reset
        </button>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 w-full max-w-7xl relative z-10">
        <div className="bg-slate-900/80 border border-slate-700 rounded-2xl shadow-2xl overflow-x-hidden backdrop-blur-md">
        ...
        <CodeMirror value={inputCode} height="420px" extensions={[javascript({jsx: true})]} theme={dracula} onChange={(value) => setInputCode(value)}/>
        </div>
        ...
        <button onClick={handleCopy} disabled={!outputCode.trim()} className="flex items-center gap-1 text-sm px-3 py-1 bg-slate-700 hover:bg-slate-600 text-white rounded-lg disabled:opacity-50">
      ...
      {
        feedback && (
          <p className={`text-center font-semibold drop-shadow-md relative z-10 ${feedback.includes('✅') || feedback.includes('copied') ? 'text-green-500' : 'text-red-500'}`}>{feedback}</p>
        )
      }
